...
- **Sample uniqueness weights (Ch.4)** — ✅ IMPLEMENTED 2026-05-21 (P1-c). `quant_engine/ml/sample_weights.py::uniqueness_weights` (AFML §4.6: `u_i = mean over [t0_i, t1_i] of 1/c_d`). Wired through `trainer.build_training_dataset` and forwarded to every RF `.fit()` via `Pipeline.fit(rf__sample_weight=…)`. Cross-symbol concurrency (master calendar = union of all label start dates). Uses `triple_barrier_events.t1_offset` for true per-label resolution time. `metadata.json` records `n_effective = (Σw)²/Σw²` — the variance-equivalent unique-obs count. Tests: `tests/test_ml_sample_weights.py` (10 assertions: domain, isolated→1, K-coincident→1/K, non-overlap→1, monotone concurrency, hand-computed reference).
